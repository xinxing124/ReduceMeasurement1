		...
		{			
			...
			if (evtMask & EV_RXCHAR) 
			{
				...
				if (actualReadLen>0)
				{
					...
					int rev;
					rev=readBuf[2]*256+readBuf[3];
					pRecvLen=actualReadLen;
					//将接收的缓冲区拷贝到pRecvBuf种
					pRecvBuf = new BYTE[pRecvLen];
					//CopyMemory(pRecvBuf,readBuf,pRecvLen);
					CeSafeCopyMemory(pRecvBuf,readBuf,pRecvLen);
				}
...
BYTE* CCESeries::ReadSyncPort(long *revlen)
{
	BYTE* buf=NULL;
	//将接收的缓冲区拷贝到pRecvBuf种
	...
	} while((dwEnd - dwStart) <= 5); 
	*revlen=0;
	return buf;
}

long CCESeries::ReadSyncPort(BYTE*buf)
{
	//将接收的缓冲区拷贝到pRecvBuf种
	DWORD dwStart = GetTickCount();
	DWORD dwEnd = 0;
	...
	} while((dwEnd - dwStart) <=3000); 

	return pRecvLen;
}
BYTE* CCESeries::Command(const BYTE* sendbuf, DWORD sendlen, DWORD* revlen)
{
	COMSTAT ComStat;
	DWORD dwErrorFlags;
	...
	bStat=WriteFile(m_hComm,sendbuf,sendlen,&sendlen,NULL);
	if(!bStat) TRACE0("写串口失败!");
	Sleep(2);
	...
	{
		//分配内存
		revbuf = new BYTE[dwLen];
		ZeroMemory(revbuf,dwLen);
		bStat=ReadFile(m_hComm,revbuf,dwLen,revlen,0);
		if(!bStat) 
		{
			TRACE0("读串口失败!");
		}
